...
export const promptTemplates: AIPromptTemplate[] = [
  // Characters
  {
    ...
    type: 'character',
    prompt:
      'Create a detailed character including: appearance, personality, background, motivations, relationships, strengths and weaknesses. Character concept: {userInput}',
  ...
  // Places
  {
    ...
    type: 'place',
    prompt:
      'Create a detailed place including: location, environment, notable features, inhabitants, history, and cultural significance. Place concept: {userInput}',
  ...
  // Objects
  {
    ...
    type: 'object',
    prompt:
      'Create a detailed object including: appearance, materials, function, history, significance, and who might use or value it. Object concept: {userInput}',
  ...
  // Events
  {
    ...
    type: 'event',
    prompt:
      'Create a detailed event including: what happened, when and where it occurred, participants, causes, consequences, and historical significance. Event concept: {userInput}',
  ...
  // Concepts
  {
    ...
    type: 'concept',
    prompt:
      'Create a detailed concept including: definition, origins, principles, how it works, cultural impact, and practical applications. Concept idea: {userInput}',
...
export async function generateWorldElement(
  ...
  try {
    ...
    const basePrompt = template
      ? template.prompt.replace('{userInput}', userPrompt)
    ...
    const result = await generateText({
      model: provider === 'openai' ? providers.openai!(modelName) : providers.anthropic!(modelName),
...
export function getApiKey(provider: 'openai' | 'anthropic'): string {
  ...
  if (!key) {
    throw new Error(
      `Missing ${provider.toUpperCase()} API key. Please add VITE_${provider.toUpperCase()}_API_KEY to your .env file.`
    )
  }
